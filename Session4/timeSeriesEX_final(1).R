...
r.sims <- matrix(rep(0,1000*84),nrow=84, ncol=1000)
e <- matrix(rnorm(84*1000, mean =mean(jj), sd = sd(jj)), 84, 1000) 
e <- matrix(rnorm(84*1000), 84, 1000) 
...
for(j in 1:1000){
  r.sims[1,j] <- r0 + ar1*(r0) - ma1*e[1,j]
  r.sims[2,j] <- r0 + ar1*(r.sims[1]-r0) - ma1*e[2,j] - ma2*e[1,j]
  for(i in 3:84){ # I replaced 1 by 3
    r.sims[i,j] <- r0 + ar1*(r.sims[i-1]-r.sims[i-2]) - ma1*e[i] - ma2*e[i-1]
  }
}

plot(ts(r.sims[1:20,1], start=1980, freq=4))
